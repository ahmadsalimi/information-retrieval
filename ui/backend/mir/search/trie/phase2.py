...
def construct_positional_indexes(dataset: str, _corpus: Corpus):
    ...
    """
    length = sum(sum(len(tokens)
                     for section, tokens in doc.items() if section != 'category')
                 for doc in _corpus.non_stop_documents.values())
    with tqdm(total=length, desc='Constructing positional indexes') as pbar:
        trie = TrieNode()
